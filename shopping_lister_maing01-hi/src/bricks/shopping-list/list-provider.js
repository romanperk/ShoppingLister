...
const initialShoppingLists = [{
  ...
  const ListProvider = createComponent({
    ...
    render(props) {
      //@@viewOn:private
      const [lists, setLists] = useState(initialShoppingLists); // State to manage multiple lists
      const [currentListId, setCurrentListId] = useState(initialShoppingLists[0]?.id); // Initialize with the ID of the first list
      ...
      function getSelectedListWithUnresolvedItems() {
        ...
        return {
          ...selectedList,
          itemList: selectedList.itemList.filter((item) => !item.resolved),
        };
      }
      ...
      function createItem(listId, item) {
        setLists((prevLists) =>
          prevLists.map((list) =>
            list.id === listId
              ? { ...list, itemList: [...list.itemList, { ...item, id: Utils.String.generateId() }] }
              : list
          )
        );
      }
